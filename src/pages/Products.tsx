...
function FiltersSidebar({ className }: { className?: string }) {
  ...
  const setPriceRange = (values: number[]) => {
    const next = new URLSearchParams(searchParams);
    next.delete('page');
    if (values[0] > 0) next.set('minPrice', String(values[0]));
    else next.delete('minPrice');
    if (values[1] < MAX_PRICE) next.set('maxPrice', String(values[1]));
  ...
  const hasFilters = activeCategories.length > 0 || minPrice > 0 || maxPrice < MAX_PRICE || inStock;

  return (
    <div className={className}>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display text-lg font-semibold">{t('products.filters')}</h3>
        {hasFilters && (
      ...
      {/* Price Range */}
      <div className="mb-6">
        <p className="text-sm font-medium mb-3">{t('products.priceRange')}</p>
        <Slider
          min={0}
          max={MAX_PRICE}
          step={50}
      ...
      <div className="flex items-center gap-2">
        <Checkbox id="in-stock" checked={inStock} onCheckedChange={toggleInStock} />
...
export default function ProductsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['products-page', sort, page, search, activeCategories.join(','), minPrice, maxPrice, inStock],
    queryFn: async () => {
      ...
      if (maxPrice < MAX_PRICE) filtered = filtered.filter((p) => p.price <= maxPrice);
